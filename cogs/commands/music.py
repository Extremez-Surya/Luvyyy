...
class PlatformSelectView(LayoutView):
    ...
    async def perform_search(self, source):
        try:
            ...
            return await self.ctx.send(view=CV2(f"{WARNING} Music server is currently unavailable. Please try again shortly."))
        if not results:
...
class Music(commands.Cog):
    ...
    async def connect_nodes(self) -> None:
        ...
        # Avoid reconnecting if an active node is already connected
    ...
    async def play_source(self, ctx, query):
        ...
        try:
            ...
            await ctx.send(view=CV2(f"{WARNING} Music node is currently connecting. Please try again in a moment."))
            return
        except Exception as e:
            await ctx.send(view=CV2(f"{WARNING} Failed to search for track: {e}"))
            return
        ...
        if isinstance(tracks, wavelink.Playlist):
            ...
            await ctx.send(view=CV2(f"{ZPLUS} Added playlist [{tracks.name}](https://discord.com/users/731390792567881739) with **{len(tracks.tracks)} songs** to the queue."))
            if not vc.playing:
            ...
            track = tracks[0]
            await vc.queue.put_wait(track)
            await ctx.send(view=CV2(f"{ZPLUS}   Added [{track.title}](https://discord.com/users/731390792567881739) to the queue."))
            if not vc.playing:
    ...
    async def handle_spotify_link(self, ctx, vc, link, type_):
        try:
            if type_ == "track":
                ...
                await ctx.send(view=CV2(f"{ZPLUS} Added all tracks from album **[{album_info['name']}](https://discord.com/users/731390792567881739)** to the queue."))
                if not vc.playing:
                    next_track = await vc.queue.get_wait()
                    await vc.play(next_track)
                    await self.display_player_embed(vc, next_track, ctx)
    ...
    async def on_wavelink_node_ready(self, payload: wavelink.NodeReadyEventPayload):
        print(f"[Lavalink] Node ready: {payload.node.identifier} ({payload.node.uri})")

    @commands.Cog.listener()
    async def on_wavelink_node_closed(self, payload: wavelink.NodeDisconnectedEventPayload):
